...
<html >
    ...
    <body>
        <%
            String temDomain = Tool.validStringRequest(request.getParameter("domain"));
            String[] domain = null;
            if (Tool.checkNull(temDomain)) {
                domain = AccSiteManager.buidSite(adminInfo.getSiteManager());
            } else {
                domain = new String[1];
                domain[0] = temDomain;
            }
            ...
            int totalPage = 0;
            int totalRow = analyDao.getCustomerCountClick(startTime, endTime, domain, locationID, adsId, 0);
            totalPage = (int) totalRow / rowPerPage;
            if (totalRow % rowPerPage != 0) {
                totalPage++;
            }
            ArrayList<AnalyticsAll> all = analyDao.getCustomerClick(currentPage, startTime, endTime, domain, locationID, adsId, rowPerPage, 0);
        %>
                            ...
                            <table id="rounded-corner" align="center">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>Tên Miền:</td>
                                        <td>
                                            <%  String[] listDoamin = AccSiteManager.buidSite(adminInfo.getSiteManager());
                                                if (listDoamin != null) {
                                            %>
                                            <select style="width: 150px" name="domain">
                                                <%
                                                    for (String one : listDoamin) {
                                                %>
                                                <option <%=one.equals(temDomain)?"selected='selected'":"" %> value="<%=one%>"><%=one%></option>
                                                <%
                                                    }
                                                %>
                                            </select>
                                ...
                                <script>
                                    function changeRowView(val) {
                                        ...
                                        location.href = "<%=request.getContextPath()%>/customer/statistic/domain/click.html?rowPerPage=" + val;
                                    }
                                </script>
                                <tr align="center">
                                    <td colspan="3">
                        ...
                        <table align="center" id="rounded-corner" summary="Gk Joint Stock Company" >
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td ><a href="<%="/customer/statistic/click/detail-" + oneAnalytic.getAdvID() + ".html"%>"> Chi tiết</td>
                                    <td></td>
                                </tr>
                                <%
