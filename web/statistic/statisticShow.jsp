...
<html >
    ...
    <body>
        <%
            ...
            int totalPage = 0;
            int totalRow = analyDao.getCountAnalyticShow(startTime, endTime, domain, locationID, adsId);
            totalPage = (int) totalRow / 50;
            if (totalRow % 50 != 0) {
                totalPage++;
            }
            ArrayList<AnalyticsAll> all = analyDao.getAnalyticShow(currentPage,50, startTime, endTime, domain, locationID, adsId);
        %>
                        ...
                        <table align="center" id="rounded-corner" summary="Gk Joint Stock Company" >
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="rounded">Lượt hiển thị</th>
                                    <th scope="col" class="rounded">Ngày hiển thị</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td align="center" style="border-right: solid 1px #00ccff"><%=DateProc.Timestamp2DDMMYYYY(oneAnalytic.getDateShow()) %></td>
                                    <td onmouseover="setTipMsg('<%=oneAnalytic.getAdvID()%>');" onmouseout="cancel();
                                            hideddrivetip();">Chi tiết</td>
                                    <td></td>
                                </tr>
                                <%
                                        count += oneAnalytic.getClick();
                                    }
                                    endTime = DateProc.Timestamp2DDMMYYYY(DateProc.createTimestamp());
                                %>
                                <tr>
                                    <td></td>
                                    <td colspan="2">Tổng lượt hiển thị</td>
                                    <td class="redBold"><%=priceWithoutDecimal(Double.valueOf(count+"")) %></td>
