...
public class Modules {
    ...
    public Logger logger = Logger.getLogger(Modules.class);
    public static final ArrayList<Modules> ALL_MODULE;
    ...
    public static int getModuleID(String resource) {
        int mid = 0;
        for (Modules one : ALL_MODULE) {
            if (one.getResource().equalsIgnoreCase(resource)) {
                mid = one.getModulID();
                break;
            }
        }
    ...
    public ArrayList listAllModule() {
        ...
        ResultSet rs = null;
        String querySQl = "select * from modules WHERE STATUS !=404";
        try {
