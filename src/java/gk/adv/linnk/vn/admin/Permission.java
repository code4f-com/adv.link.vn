...
public class Permission {
    ...
    public ArrayList getRoleGroupModule(int gId) {
        ...
        ResultSet rs = null;
        String sql = "select M.MODULE_ID,M.RESOURCE,M.NAME,G.ID,G.GROUP_ID,G.ALL_RIGHT,G.VIEW_RIGHT,G.ADD_RIGHT,G.EDIT_RIGHT,G.DEL_RIGHT "
    ...
    public ArrayList getRoleAccModule(int accID) {
        ...
        String sql = "select M.MODULE_ID,M.RESOURCE,M.NAME"
                + " ,U.ID,U.ACC_ID,U.ALL_RIGHT,U.VIEW_RIGHT,U.ADD_RIGHT,U.EDIT_RIGHT,U.DEL_RIGHT "
        ...
        try {
            ...
            for (Permission oneGper : perGroups) {
                ...
                if (rs.next()) {
                    ...
                    if (tem == 1) {
                        // CO roi
                        pstm = conn.prepareStatement("UPDATE user_permission SET ALL_RIGHT = ?, VIEW_RIGHT = ?,ADD_RIGHT = ?,EDIT_RIGHT=?,DEL_RIGHT=? WHERE ACC_ID = ? AND MODULE_ID = ?");
                        ...
                        // Chua co
                        pstm = conn.prepareStatement("INSERT INTO user_permission(ACC_ID,MODULE_ID,ALL_RIGHT,VIEW_RIGHT,ADD_RIGHT,EDIT_RIGHT,DEL_RIGHT)"
                                + "VALUES(?,?,?,?,?,?,?)");
                        pstm.setInt(i++, uid);
                        pstm.setInt(i++, oneGper.getModelId());
                        pstm.setBoolean(i++, oneGper.isSpecial());
    ...
    public static void removeUserRole(int gid, int uid) {
        ...
        try {
            ...
            for (Permission oneGper : perGroups) {
                ...
                DBPool.releadRsPstm(rs, pstm);
                int i = 1;
                pstm = conn.prepareStatement("UPDATE user_permission SET ALL_RIGHT = 0, VIEW_RIGHT = 0,ADD_RIGHT = 0,EDIT_RIGHT=0,DEL_RIGHT=0 WHERE ACC_ID = ? AND MODULE_ID = ?");
                // CO roi
                pstm.setInt(i++, uid);
                pstm.setInt(i++, oneGper.getModelId());
                pstm.execute();
        ...
        try {
            ...
            if (gDao != null && gDao.getAllAccId().size() > 0) {
                ...
                for (Permission onePerGroup : perGroups) {
                    String sql = "UPDATE USER_PERMISSION SET ALL_RIGHT=?,VIEW_RIGHT=?,ADD_RIGHT=?,EDIT_RIGHT=?,DEL_RIGHT=?"
                            + " where MODULE_ID = ? ";
