 ...
 *
 * The pool must be initialized prior to use. This should typically be early
 * on<br/>
 ...
 *		pool.initialize();
 *	}
 * </pre> The easiest manner in which to initialize the pool is to set the
 * servers and rely on defaults as in the first example.<br/>
...
public class SockIOPool {
    ...
    public static class SockIO implements LineInputStream, AutoCloseable {
        ...
        private static final Logger log = Logger.getLogger(SockIO.class.getName());

        // Tạo Cleaner dùng chung cho các instance của SockIO
        ...
        // Quản lý việc dọn dẹp tài nguyên ngầm
        private final Cleaner.Cleanable cleanable;

        /**
         * Lớp tĩnh chứa logic dọn dẹp Socket. BẮT BUỘC phải là static class để
         * tránh giữ tham chiếu vòng (memory leak) đến SockIO.
         */
        private static class ResourceCleaner implements Runnable {

            private Socket socketToClose;

            ResourceCleaner(Socket socketToClose) {
            ...
            public void run() {
                if (socketToClose != null && !socketToClose.isClosed()) {
                    try {
                        log.error("++++ closing potentially leaked socket in Cleaner");
                        socketToClose.close();
                    } catch (Throwable t) {
                        log.error("Error closing leaked socket in Cleaner: " + t.getMessage(), t);
                    } finally {
                        socketToClose = null;
                    }
                }
        ...
        public SockIO(SockIOPool pool, String host, int port, int timeout, int connectTimeout, boolean noDelay)
            ...
            // Đăng ký dọn dẹp với Cleaner thay thế cho finalize()
            this.cleanable = CLEANER.register(this, new ResourceCleaner(this.sock));
        }
        ...
        public SockIO(SockIOPool pool, String host, int timeout, int connectTimeout, boolean noDelay)
            ...
            this.sock = getSocket(ip[0], Integer.parseInt(ip[1]), connectTimeout);
            ...
            // Đăng ký dọn dẹp với Cleaner
            this.cleanable = CLEANER.register(this, new ResourceCleaner(this.sock));
        }

        protected static Socket getSocket(String host, int port, int timeout) throws IOException {
            SocketChannel sockChannel = SocketChannel.open();
            sockChannel.socket().connect(new InetSocketAddress(host, port), timeout);
            return sockChannel.socket();
        }
        ...
        public void trueClose(boolean addToDeadPool) throws IOException {
            ...
            // Hủy đăng ký Cleaner vì tài nguyên đã/sắp được đóng chủ động
            if (cleanable != null) {
                cleanable.clean();
            }
            ...
            if (in != null) {
                try {
                    ...
                    log.error("++++ error closing input stream for socket: " + toString() + " for host: " + getHost(), ioe);
                    errMsg.append("++++ error closing input stream for socket: ").append(toString()).append(" for host: ").append(getHost()).append("\n");
            ...
            if (out != null) {
                try {
                    ...
                    log.error("++++ error closing output stream for socket: " + toString() + " for host: " + getHost(), ioe);
                    errMsg.append("++++ error closing output stream for socket: ").append(toString()).append(" for host: ").append(getHost()).append("\n");
                    errMsg.append(ioe.getMessage());
                    err = true;
                }
            }
            ...
            if (sock != null && !sock.isClosed()) {
                try {
                    ...
                    log.error("++++ error closing socket: " + toString() + " for host: " + getHost(), ioe);
                    errMsg.append("++++ error closing socket: ").append(toString()).append(" for host: ").append(getHost()).append("\n");
                    errMsg.append(ioe.getMessage());
                    err = true;
                }
            }

            if (addToDeadPool && pool != null) {
        ...
        /**
         * Cho phép class implement AutoCloseable. Khi gọi close() sẽ trả
         * connection lại cho Pool.
         */
        @Override
        public void close() {
            if (log.isDebugEnabled()) {
