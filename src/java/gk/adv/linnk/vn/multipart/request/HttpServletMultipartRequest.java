package gk.adv.linnk.vn.multipart.request;
...
/**
 * A MultipartRequest HttpServletRequest wrapper. This class extends the
 * HttpServletRequestWrapper class and where an multipart form is encountered
 * will parse the input stream of the uploaded data, and provide implementations
 * of the HttpServletRequest methods, as well as file specific methods:
 * 
 * <ul>
 ...
 * 
 * <p>
 * This class can be used for all http requests, there is no need to check if
 * the ENCTYPE is for a file upload form. In the case where its a normal upload,
 * this class call the superclass methods.
 * </p>
 * 
 * <pre>
 * request = new HttpServletMultipartRequest(request);
 * 
 * for (Enumeration e = ((HttpServletMultipartRequest) request)
 * 		.getFileParameterNames(); e.hasMoreElements();) {
 * 	String name = (String) e.nextElement();
 * 	MultipartFile multiFile = ((HttpServletMultipartRequest) request)
 * 			.getFileParameter(name);
 * }
 * </pre>
 * 
 * Currently does not recognise nested Content-type: multipart/mixed;
 * 
 * The Apache File Upload parser does support this:
 * http://issues.apache.org/jira/browse/FILEUPLOAD-62
 * 
 * @author Jason Pell
 * 
 * @version 2.00b11
...
//	public static final String DEFAULT_ENCODING = "ISO-8859-1";
	 ...
	 */
	public static final long MAX_CONTENT_LENGTH = 150 * (1024 * 1024); //150MB!
	 ...
	 * @throws IOException
	 */
	public HttpServletMultipartRequest(HttpServletRequest request)
			throws IOException {
	...
	public String toHtmlString() {
		...
		if (isMultipartRequest()) {
			sbReturn.append("<li>Max Content Memory Threshold: ").append(getMaxContentLengthMemoryThreshold()).append("</li>");
			...
			if (fileEnumeration.hasMoreElements()) {
				...
				for (; fileEnumeration.hasMoreElements();) {
					...
					for (int i = 0; i < multiFiles.length; i++) {
						...
						sbReturn.append("\n<tr>" + "<td>").append(strName).append("</td>" + "<td>").append(multiFile.getName() != null ? multiFile
                                                                                                                                                .getName() : "").append("</td>");

						sbReturn.append("<td>").append(multiFile.getContentType() != null ? multiFile
                                                                                            .getContentType()
                                                                                            : "").append("</td>" + "<td>").append(multiFile.getSize() != -1 ? multiFile
                                                                                                                                               .getSize()
                                                                                                                                               + ""
                                                                                                                                               : "")
								.append("</td>" + "</tr>");
				...
				if (isMaxLengthExceeded()) {
					sbReturn.append("<p style=\"{color: red;}\">Max Bytes exceeded (").append(m_contentLength).append(" > ").append(m_maxContentLength)
	...
	private void parse(InputStream in) throws IOException {
		...
		strLine = read > 0 ? new String(m_blockOfBytes, 0, read, m_charEncoding)
				: null;
	 ...
	 * 
	 * @param strName -
	 *            Url parameter this file was loaded under.
